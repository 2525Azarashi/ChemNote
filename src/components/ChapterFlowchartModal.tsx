...
export function ChapterFlowchartModal({ 
  ...
  return (
    <div className="fixed inset-0 bg-[#1B2631]/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 z-[100] overflow-y-auto">
      <motion.div 
        initial={{ opacity: 0, scale: 0.95, y: 15 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.95, y: 15 }}
        transition={{ duration: 0.25, ease: "easeOut" }}
        className="bg-white rounded-3xl shadow-2xl border border-gray-150 w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden"
      >
        {/* Header */}
        <div className="p-4 sm:p-6 border-b border-gray-100 flex items-center justify-between bg-white text-[#2C3E50] shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-gray-500 font-modern">
                学習フローチャート・ロジックツリー
              </p>
            </div>
          </div>
        ...
        {/* Content Body */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 bg-[#FDFBF7]">
          <div className="bg-white rounded-2xl border border-gray-200 p-2 sm:p-4 shadow-sm w-full">
            <InteractiveTree 
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-100 flex justify-between items-center shrink-0">
          ...
          <button
            onClick={onClose}
            className="px-4 py-2 bg-[#2C3E50] hover:bg-[#1B2631] text-white text-xs sm:text-sm font-bold rounded-xl shadow-xs transition-colors"
          >
            閉じる
          </button>
        </div>
