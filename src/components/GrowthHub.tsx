...
import { ArrowLeft, Coins, BookOpen, Swords, Award, Store, Target, Shirt, BarChart3 } from 'lucide-react';
...
export type GrowthPage = 'overview' | 'shop' | 'outfit' | 'badges' | 'stats' | 'missions';
export function GrowthHub({ page, onPage, onBack, onBattle, onReview }: {
  ...
  return <div ref={scroll} className="mana-hub h-full min-h-0 overflow-y-auto pb-app-nav" data-growth-hub>
    <header className="mana-hub-header">
      ...
      <button type="button" onClick={() => onPage('overview')}><Coins size={18} /> マイページ</button>
    </header>
    {page === 'overview' ? <main className="mana-hub-main">
      <p className="mana-eyebrow">MY GROWTH & COLLECTION</p>
      <h1>学ぶ。ためる。自分らしく。</h1>
      <p className="mb-5 text-sm text-slate-600">対戦も、復習も、とびら君の成長につながる。</p>
      <GrowthHomeStrip expanded onProfile={() => onPage('outfit')} onShop={() => onPage('shop')}
      ...
      <section className="mana-hub-card" aria-label="マナコインのため方">
        ...
        <div className="mana-earn-actions">
          ...
          {onBattle && <button type="button" onClick={onBattle}><Swords size={20} /><strong>対戦でためる</strong><small>AI対戦も対象／報酬は受取式</small></button>}
        </div>
      ...
      {page === 'missions' ? <BattleMissions key={uid} standalone onBack={() => onPage('overview')} onBattle={onBattle} onReview={onReview} onShop={() => onPage('shop')} />
        : <BattleProfile key={`${uid}:${page}`} standalone initialTab={page} onBack={() => onPage('overview')} />}
    </>}
  </div>;
}
