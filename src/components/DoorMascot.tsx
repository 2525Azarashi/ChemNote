...
const chemistryFacts = [
  ...
  // ── 共通テスト・試験で使えるテクニック ──
  '【テク】単位を必ず書いて計算しよう。単位がそろえば計算式の形は自然と決まります（次元解析）。',
  '【テク】有効数字は「与えられた数値の最小桁数」に合わせる。最後にまとめて四捨五入するとミスが減ります。',
  '【テク】中和滴定は「酸の価数×モル＝塩基の価数×モル」。この1本の式に当てはめれば多くの問題が解けます。',
  '【テク】気体の問題は PV=nRT を軸に。R=8.31×10³ [Pa·L/(K·mol)]、温度は必ずK（℃+273）で。',
  '【テク】「水溶液の希釈」では溶質の物質量は不変。C₁V₁=C₂V₂ ですばやく処理できます。',
  '【テク】化学反応式の係数は質量保存（原子の数）で合わせる。係数比＝物質量比＝（気体なら）体積比。',
  '【テク】グラフ問題は「軸・傾き・切片・変化点」に着目。傾きが何を表すか先に考えると速い。',
  '【テク】選択肢問題は「明らかに違う選択肢」から消去。極端な値や単位ミスの選択肢は外しやすい。',
  '【テク】中和点の前後でpHが急変（中和滴定曲線）。指示薬は変色域がpHジャンプに重なるものを選ぶ。',
  '【テク】酸化還元の量的関係は「やり取りした電子の物質量が等しい」。半反応式を書くと立式が安定します。',
  '【テク】mol計算は「与えられた量→mol→求めたい量」の3ステップ。途中をmolに統一すると迷いません。',
  '【テク】溶解度や濃度の問題は「溶質・溶媒・溶液」のどれを指すか必ず確認。質量パーセント濃度との混同に注意。',
  '【テク】共通テストは時間勝負。計算が重い問題は後回しにして、知識問題で確実に得点しよう。',
  '【テク】「正しいものを選べ」か「誤っているものを選べ」か、設問の指示に必ず印をつけてから解こう。',
  '【テク】炎色反応・気体の性質・沈殿の色は暗記が得点に直結。スキマ時間に語呂で覚えるのが効率的。',
];
