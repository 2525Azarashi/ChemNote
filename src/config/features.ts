...
export const FEATURES = {
   ...
   * 高校入試 理科：★true（公開）★
   *
   * ■ これは「科目」だが SUBJECT_FEATURE_KEY には入れない
   *   下の対応表は「本体の教科（章→大問→小問の形を持つもの）」用で、
   *   そこに載せると科目選択画面のカードに並ぼうとする。
   *   理科は本体のその形を持っておらず、演習も単元の絞り込みも
   *   理科の画面が自分で持っている（src/features/rika/）。
   *   カードに並べると、押した先で1問も出せない画面に入ってしまう。
   *   ★だから科目一覧ではなく、ホームの独立した入口として出す。★
   *
   * ■ 公開してよいと判断した根拠（実測）
   *   ・対戦の出題プール 1117 問（全4択・採点は選択肢の一致だけなので確実に動く）
   *   ・演習・まとめ・出題傾向の3画面が揃っている
   *   ・起動時の重さは増えていない（3画面すべて遅延読み込み。
   *     tests/screenDataWeight.test.ts が main.tsx から
   *     features/rika へ静的に辿れないことを見張っている）
   *
   * ■ 隠すときは4箇所すべてを見ること
   *   1. ホームの入口カード（src/components/Home.tsx の onRika）
   *   2. ルーティング（src/App.tsx の appState === 'rika' の描画）
   *   3. 対戦結果からの遷移（src/App.tsx の handlePracticeFromBattle）
   *   4. 対戦の教科一覧（src/battle/core/battleRules.ts の rika.enabled）
   *   ★4 は別のフラグ（対戦ルール側）で持っている★
