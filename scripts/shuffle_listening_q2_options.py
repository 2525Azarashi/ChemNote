...
#   (15, 2) 家族写真の中から母親を選ぶ問。
#          S10Q2 の見出しの絵が ①左端の女性・②赤ちゃんを抱く女性・
#          ③背の高い男性・④①の後ろに立つ女性 で4つとも一致。
#
# ここに登録した「値」の位置にある絵は、その問には使わない。
# 例：S15Q2 の見出しの絵（靴箱の壁）は第15セット問2 には使わず捨てる。
...
#
# これらの問は実物イラストを使わないので、並べ替えの対象に戻してある
# （＝後から自前生成の絵を入れ替え後の並びで作れる）。
EXCLUDED_PDF_ILLUSTRATIONS = frozenset(
    {(6, 3), (11, 1), (13, 1), (7, 3), (8, 3), (16, 2)}
)
