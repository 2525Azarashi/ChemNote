...
describe('画面ごとの起動時の重さ（教科データを読み込みすぎていないか）', () => {
  ...
  it('★アプリ全体（main.tsx）の起動時 src/data 読み込みが悪化していない★', () => {
     ...
     */
    /*
     *   13 件 /   692,101 B … 本体に教科データを持たない教科（高校入試 理科）の
     *                          登録簿 src/data/externalSubjects.ts を足した後。
     *
     *     ★ここで件数を1つ上げた理由と、上げるだけで済ませなかった話★
     *
     *     この門はファイル件数だけを見ていたので、葉を1枚足しただけで落ちた。
     *     「じゃあ 13 にする」で済ませると、次に誰かが
     *     ★重いファイルを1枚足したときも同じ顔で 14 に上げてしまう★。
     *     件数は構造の目安にはなるが、重さそのものではない。
     *
     *     そこで数え方を足した（過去にも同じ判断をしている。
     *     コメントを重さと数えていた件を「予算を緩める」ではなく
     *     「数え方を直す」で解いた、上の codeBytesOf の記録を参照）。
     *
     *       (a) 件数        … 構造が増えていないか（これまでどおり）
     *       (b) 実コード量  … ★新設★ 本当に重くなっていないか
     *       (c) 増えた分が葉か … ★新設★ その1枚から先へ辿れないこと
     *
     *     実測では件数が 12 → 13 に増えた一方で、
     *     実コード量は 730,467 → 692,101 B と★減っている★。
     *     件数だけを見ていると「悪化した」と読めてしまうが、
     *     配信量は増えていない。(b) があるとこの区別がつく。
     */
    const CURRENT = 13;
     ...
     * (b) ★実コード量の上限（この回で新設）★
     *
     * 件数の門は「1枚あたりの重さ」を見ていない。
     * 索引だけを読む軽い葉と、問題データ本体は同じ「1件」に見える。
     * 実測 692,101 B の少し上に置くので、
     * 教科データ本体（1ファイルで数十万バイト）が戻れば必ず超える。
     *
     * ★この数字は上限であって目標ではない★
     * 軽くする作業が進んだら下げる（下げれば後戻りも検出できる）。
     */
    const BYTES_BUDGET = 700_000;
    expect(
      dataBytes,
      `★起動時の src/data 実コード量が ${dataBytes.toLocaleString()} B（上限 ${BYTES_BUDGET.toLocaleString()} B）★\n` +
        'ファイル件数が同じでも、1枚が重くなればここが落ちる。\n' +
        `内訳:\n  ${dataFiles.map((f) => `${codeBytesOf(join(ROOT, f))} ${f}`).join('\n  ')}`,
    ).toBeLessThanOrEqual(BYTES_BUDGET);

    /*
     * (c) ★件数を1つ上げた相手が本当に葉であることの確認（この回で新設）★
     *
     * 上限を 13 に上げたのは externalSubjects.ts のぶんである。
     * この1枚から先へ辿れてしまうなら、上げた根拠が消える。
     * 名前を許可リストに載せて終わりにせず、
     * ★許可した相手の性質そのものを毎回測る★。
     * （tests/subjectLabels.test.ts で同じ考え方を使っている）
     */
    const leaf = reachFrom('src/data/externalSubjects.ts');
    expect(
      leaf.files,
      '★externalSubjects.ts が葉でなくなっている★\n' +
        'この1枚は「教科の名前と色と単元名」だけを持つ約束で、\n' +
        'だからこそ起動時に読んでよいと判断して上限を1つ上げた。\n' +
        '何かを import した時点でその根拠が消えるので、\n' +
        '上限を上げるのではなく import を外すこと。\n' +
        `到達しているファイル:\n  ${leaf.files.join('\n  ')}`,
    ).toEqual(['src/data/externalSubjects.ts']);
  });
