...
describe('L3: ホームの CTA は order で構造的に画面内へ', () => {
  ...
  it('★パソコンで「はみ出しを隠す」指定を復活させない★', () => {
    /*
      -----------------------------------------------------------------
      この検査を足した理由（実際に起きた不具合）
      -----------------------------------------------------------------
      以前は「パソコンでは1画面に収める」ため、
      パソコン幅のときだけ はみ出しを隠す指定を入れていた。

      ところが縦が短いパソコン（ノートPCに多い）では
      中身が1画面に収まらず、
      ★隠れた部分に手が届かない＝下まで読めない★状態になっていた。

      実際に測った結果：
        横1280×縦720 … 83px ぶん届かない
        横1366×縦768 … 59px ぶん届かない
        横1440×縦900 … 収まっている（問題なし）
        横1920×縦1080 … 収まっている（問題なし）

      「自分の画面では起きない」ため、目で見て気づくことができない。
      だから機械に見張らせる。

      収まっているときの見た目は変わらない（auto は必要なときだけ
      スクロールバーを出す）ので、1画面設計そのものは維持している。
      -----------------------------------------------------------------
    */
