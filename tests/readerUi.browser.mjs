...
try {
  ...
  await page.evaluate(async()=>{
    ...
    window.print=()=>{
      window.lastPrint={classes:document.body.className,title:document.title,modal:!!document.querySelector('dialog[open]'),disclosures:[...document.querySelectorAll('.learning-content details')].map(d=>d.open)};
  ...
  for (const viewport of [{width:320,height:568},{width:390,height:844},{width:667,height:375},{width:1280,height:900}]) {
    ...
    for (const [subject,initialTab] of sources) {
      ...
      if (viewport.width<640) {
        const article=await page.locator('.learning-content').boundingBox();
        assert.ok(article.width>=viewport.width-32,`${subject} narrow paper: ${article.width}`);
        // A paragraph inside a teaching box legitimately has that box's padding.
        assert.ok(metrics.width>=viewport.width-72,`${subject} excessive nested padding: ${metrics.width}`);
      }
      await fit(subject);
      ...
      for (const mode of ['answers','blank']) {
        ...
        assert.ok(snap.disclosures.every(open=>open===(mode==='answers')));
        assert.equal(await page.locator('.learning-content details[open]').count(),0);
