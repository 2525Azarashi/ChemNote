...
describe('actual live/review rendering contracts',()=>{
 ...
 it.each([7600,6600,5600,4600,3600,2600,1600,600])('does not mount questions during preparation %s',preStartMs=>{
  const html=live({preStartMs});
  expect(html).toContain('紙と筆記用具を');expect(html).toContain('用意してください');
  expect(html).not.toContain('id="battle-question"');expect(html).not.toContain('choose');
  expect(html).not.toContain('arena-opponent-answer');
 });
...
describe('combat animation outcome privacy', () => {
 it('guards without revealing opponent correctness before reveal',()=>{
  const html=live({answered:true});
  expect(html).toContain('data-combat="guard"');expect(html).not.toContain('DAMAGE');
 });
 it('clashes only for two confirmed correct answers',()=>{
  expect(live({reveal:true,myScore:score})).toContain('data-combat="clash"');
  expect(live({reveal:true})).toContain('data-combat="hurt"');
  expect(live({reveal:true,myScore:score,opponentScore:null})).toContain('data-combat="strike"');
  expect(live({reveal:true,opponentScore:null})).toContain('data-combat="miss"');
 });
});
