...
> ## ★これは詳しい版（技術的な背景つき）です★
> ## 作業だけしたい場合は **[docs/いまやること.md](./いまやること.md)** を見てください。
>
> 今やる作業は **「ルールの文章を貼り替える」1つだけ**で、3分で終わります。
> こちらの文書は、その背景や別のやり方（CLI など）も含めた詳しい版です。
>
> ### ★索引（インデックス）のデプロイは不要になりました★
> 以前この文書は `firebase deploy --only firestore:indexes` を
> 必須の手順として書いていましたが、**開発ツールの導入が必要**で
> 手間がかかるため、**索引が無くても動くようにプログラム側を書き換えました。**
>
> Firestore は「条件が1つだけの検索」なら索引を自動で作るので、
> クエリを条件1つの形に直せば索引が要りません。
> （参考: <https://firebase.google.com/docs/firestore/query-data/index-overview>）
>
> 以下の「手順①'」は**やらなくてよい**（将来の参考として残してあります）。
...
```
   ...
   ② 公開できたことを確認する
              ↓
   ③ FEATURES.battle = true にする（★すでに設定済み★）
```
...
> の 2 つが起きた。
>
> **原因1（索引）**: Firestore は **ルールと索引が別物**で、
> 別々にデプロイが必要だった。この手順書にその手順が無かった。
> → **索引が要らない形にプログラムを書き換えて解決した**ので、
>   今は索引のデプロイは不要である。
>
> **原因2（入室）**: 入室しようとする人が部屋を**読めなかった**。
> → ルールを修正した。だから**ルールの貼り替えが必要**である。
...
# 手順①' 索引（インデックス）を公開する ★← 今は不要★

> ## ★この手順はやらなくてよくなりました★
>
> 索引のデプロイにはパソコンへの開発ツール導入が必要で手間がかかるため、
> **索引が無くても動くようにプログラム側を書き換えました。**
>
> - `findOrEnqueue`：条件1つの検索にして、並べ替えは手元で行う
> - `watchMatched`：同じく条件1つの検索にする
>
> Firestore は条件1つの検索なら索引を**自動で作ります**。
> したがって**この手順を飛ばして問題ありません。**
>
> 以下は「将来、対戦する人が大幅に増えて検索が重くなったとき」
> のための参考として残します。
...
| ★全国対戦で `The query requires an index.`★ | 索引が要る形のクエリを使う**古いアプリ**を開いている | 最新のアプリをデプロイする（PR #116 をマージ）。索引の作成は不要 |
...
| # | やること | 所要 | 必須か |
|---|---|---|---|
| 1 | `firestore.rules`（1988行）を Console に貼って公開 | 3分 | ★必須★ |
| 2 | 確認（`battle_ranking` が 200 になる） | 1分 | 推奨 |
| 3 | 最新のアプリをデプロイ（PR をマージ） | — | ★必須★ |
| — | ~~索引のデプロイ~~ | — | **不要**（プログラム側で解決済み） |

`FEATURES.battle = true` は**すでに設定済み**なので触らなくてよい。

**手順1のやり方をいちばん簡単に書いたもの → [docs/いまやること.md](./いまやること.md)**
